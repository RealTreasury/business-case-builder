...
/**
 * Run sensitivity analysis on ROI inputs.
 *
 * @return void
 */
function rtbcb_ajax_sensitivity_analysis() {
    if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'rtbcb_roi_calculator_test' ) ) {
        wp_send_json_error( [ 'message' => __( 'Security check failed.', 'rtbcb' ) ], 403 );
    }
    ...
    if ( empty( $roi_data ) || empty( $base_roi ) ) {
        wp_send_json_error( [ 'message' => __( 'Missing data for sensitivity analysis.', 'rtbcb' ) ], 400 );
    }

    try {
        $inputs   = rtbcb_convert_form_data_to_inputs( $roi_data );
        $results  = rtbcb_perform_sensitivity_analysis( $inputs, $base_roi );
        wp_send_json_success( $results );
    } catch ( Exception $e ) {
        wp_send_json_error( [ 'message' => $e->getMessage(), 'code' => 'sensitivity_analysis_failed' ] );
    }
}

/**
 * Map ROI form data to calculator inputs.
 *
 * @param array $form_data Form data from frontend.
 * @return array
 */
function rtbcb_convert_form_data_to_inputs( $form_data ) {
    return [
        ...
        'total_daily_hours'    => floatval( $form_data['roi-hours-reconciliation'] ?? 0 ) + floatval( $form_data['roi-hours-reporting'] ?? 0 ) + floatval( $form_data['roi-hours-analysis'] ?? 0 ),
        'annual_wire_fees'     => floatval( $form_data['roi-wire-transfer-volume'] ?? 0 ) * floatval( $form_data['roi-avg-wire-fee'] ?? 0 ) * 12,
        'annual_error_cost'    => floatval( $form_data['roi-error-frequency'] ?? 0 ) * floatval( $form_data['roi-avg-error-cost'] ?? 0 ) * 52,
    ];
}
 ...
 * @return array
 */
function rtbcb_perform_sensitivity_analysis( $base_inputs, $base_roi ) {
    $variables = [
    ...
    $results             = [ 'base_roi' => $base_roi_percentage, 'sensitivity' => [] ];

    foreach ( $variables as $variable ) {
        $base_value = $base_inputs[ $variable ];
        $var_results = [ 'base_value' => $base_value, 'base' => $base_roi_percentage ];
        $changes = [];

        foreach ( $variations as $variation ) {
            $modified               = $base_inputs;
            $modified[ $variable ]  = $base_value * ( 1 + $variation );
            $quick_roi              = rtbcb_quick_roi_calculation( $modified );
            $label                  = ( $variation > 0 ? '+' : '' ) . ( $variation * 100 ) . '%';
            $var_results[ $label ]  = round( $quick_roi, 1 );
            $changes[]              = abs( $quick_roi - $base_roi_percentage ) / abs( $variation * 100 );
        }

        $var_results['sensitivity']      = round( array_sum( $changes ) / count( $changes ), 2 );
        $results['sensitivity'][ $variable ] = $var_results;
    }
...
/**
 * Quick ROI calculation used during sensitivity analysis.
 *
 * @param array $inputs Modified inputs.
 * @return float
 */
function rtbcb_quick_roi_calculation( $inputs ) {
    ...
    $benefits = $inputs['total_daily_hours'] * $hourly_labor_cost * 365 * 0.25 +
        ...
        $inputs['error_frequency'] * $inputs['avg_error_cost'] * 52 * 0.5;

    $costs = 50000 * rtbcb_get_company_size_multiplier( $inputs['company_size'] ) * 1.9;
 ...
 * @return float
 */
function rtbcb_get_company_size_multiplier( $company_size ) {
    $multipliers = [
        'startup'    => 0.5,
        'small'      => 0.8,
        'medium'     => 1.0,
