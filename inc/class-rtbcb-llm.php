...
SYSTEM;
	...
	private function call_openai_with_retry( $model, $prompt, $max_output_tokens = null, $max_retries = null ) {
		...
		$global_timeout_cap = intval( $this->gpt5_config['max_total_timeout'] ?? 300 );

		for ( $attempt = 1; $attempt <= $max_retries; $attempt++ ) {
			$elapsed = microtime( true ) - $start_time;
			if ( $elapsed >= $global_timeout_cap ) {
				$this->gpt5_config['timeout'] = $base_timeout;
				return new WP_Error( 'llm_timeout', __( 'The language model request timed out.', 'rtbcb' ) );
			}

			$remaining_time                = $global_timeout_cap - $elapsed;
			$this->gpt5_config['timeout'] = min( $current_timeout, (int) $remaining_time );
			...
			if ( $attempt < $max_retries ) {
				...
				$current_timeout = min( $current_timeout + 5, $global_timeout_cap );

				$delay  = min( pow( 2, $attempt - 1 ), 30 );
				$jitter = wp_rand( 0, 1000 ) / 1000;
				$sleep  = min( $delay + $jitter, $remaining_time );
				if ( $sleep > 0 ) {
	 ...
	 *
	 * @param WP_Error $error Error returned from the OpenAI API.
	...
	private function is_retryable_error( $error ) {
		...
		$status_code = is_array( $error_data ) && isset( $error_data['status'] ) ? intval( $error_data['status'] ) : null;

		$non_retryable = [ 'no_api_key', 'empty_prompt', 'llm_timeout' ];
		if ( in_array( $code, $non_retryable, true ) ) {
			return false;
		}

		if ( 'llm_http_status' === $code && $status_code && $status_code < 500 ) {
			return false;
		}
	...
	private function call_openai( $model, $prompt, $max_output_tokens = null ) {
		...
		if ( ! empty( $this->gpt5_config['stream'] ) ) {
			$args['stream']   = true;
			$args['blocking'] = false;
		}
