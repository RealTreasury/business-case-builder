...
class RTBCB_LLM {
    ...
    /**
     * Generate a comprehensive business case structure with detailed sections.
     *
     * Builds on the basic narrative generated by {@see generate_business_case}
     * and expands it using ROI data and user inputs to populate operational and
     * financial analyses that power professional reports.
     *
     * @param array $user_inputs   User provided inputs.
     * @param array $roi_data      ROI calculation results.
     * @param array $context_chunks Context chunks for RAG.
    ...
    public function generate_comprehensive_business_case( $user_inputs, $roi_data, $context_chunks = [] ) {
        ...
        $recon_hours = floatval( $user_inputs['hours_reconciliation'] ?? 0 );
        $cash_hours  = floatval( $user_inputs['hours_cash_positioning'] ?? 0 );
        $total_hours = $recon_hours + $cash_hours;

        $efficiency_rating = __( 'Good', 'rtbcb' );
        if ( $total_hours > 20 ) {
            $efficiency_rating = __( 'Poor', 'rtbcb' );
        } elseif ( $total_hours > 10 ) {
            $efficiency_rating = __( 'Fair', 'rtbcb' );
        }

        $assessment = [
            'efficiency_rating'    => $efficiency_rating,
            'benchmark_comparison' => sprintf(
                __( '%s spends %d manual hours weekly on treasury operations.', 'rtbcb' ),
                $company_name ?: __( 'The company', 'rtbcb' ),
                $total_hours
            ),
            'capacity_utilization' => sprintf(
                __( 'Manual tasks consume %d%% of available capacity.', 'rtbcb' ),
                min( 100, round( ( $total_hours / 40 ) * 100 ) )
            ),
        ];

        $pain_points         = array_map( 'sanitize_text_field', $user_inputs['pain_points'] ?? [] );
        $process_inefficiencies = [];
        foreach ( $pain_points as $pain_point ) {
            $process_inefficiencies[] = [
                'process'     => $pain_point,
                'impact'      => __( 'High', 'rtbcb' ),
                'description' => sprintf(
                    __( '%s relies on manual effort leading to delays and errors.', 'rtbcb' ),
                    $pain_point
                ),
            ];
        }
        ...
        if ( $recon_hours > 0 ) {
            $automation_opportunities[] = [
                'area'                => __( 'Bank Reconciliation', 'rtbcb' ),
                'complexity'          => __( 'Medium', 'rtbcb' ),
                'potential_hours_saved' => round( $recon_hours * 0.3, 1 ),
            ];
        }
        if ( $cash_hours > 0 ) {
            $automation_opportunities[] = [
                'area'                => __( 'Cash Positioning', 'rtbcb' ),
                'complexity'          => __( 'Medium', 'rtbcb' ),
                'potential_hours_saved' => round( $cash_hours * 0.3, 1 ),
            ];
        }
        ...
        // ----- Financial Modeling -----
        $base_benefit   = floatval( $roi_data['base']['total_annual_benefit'] ?? 0 );
        $base_roi_pct   = floatval( $roi_data['base']['roi_percentage'] ?? 0 );
        $estimated_cost = $base_roi_pct > 0 ? $base_benefit * 100 / $base_roi_pct : 0;

        $financial_metrics = [
            'roi_3_year'     => $base_roi_pct * 3,
            'payback_months' => $base_roi_pct > 0 ? round( 1200 / $base_roi_pct ) : 0,
            'npv_10_percent' => (int) round(
                ( $base_benefit / 1.1 ) + ( $base_benefit / ( 1.1 ** 2 ) ) + ( $base_benefit / ( 1.1 ** 3 ) ) - $estimated_cost
            ),
        ];

        $benefit_multipliers = [ 0.7, 1.0, 1.1, 1.2, 1.3 ];
        $cost_multipliers    = [ 1.2, 0.5, 0.4, 0.4, 0.4 ];
        $five_year_projection = [];
        foreach ( range( 1, 5 ) as $i ) {
            $benefits = $base_benefit * $benefit_multipliers[ $i - 1 ];
            $costs    = $estimated_cost * $cost_multipliers[ $i - 1 ];
            $five_year_projection[ 'year_' . $i ] = [
                'benefits'  => $benefits,
                'costs'     => $costs,
        ...
        $implementation_roadmap = [
            [
                'phase'       => __( 'Planning', 'rtbcb' ),
                'timeline'    => __( 'Month 1', 'rtbcb' ),
                'description' => __( 'Finalize requirements and select vendor', 'rtbcb' ),
            ],
            [
                'phase'       => __( 'Execution', 'rtbcb' ),
                'timeline'    => __( 'Months 2-3', 'rtbcb' ),
                'description' => __( 'Configure system and integrate banks', 'rtbcb' ),
            ],
            [
                'phase'       => __( 'Go-Live', 'rtbcb' ),
                'timeline'    => __( 'Month 4', 'rtbcb' ),
                'description' => __( 'User training and production rollout', 'rtbcb' ),
            ],
        ];

        // ----- Industry Benchmarks & Vendor Evaluation -----
        $industry_benchmarks = [
            'industry_multiplier' => $roi_data['base']['assumptions']['industry_benchmark'] ?? 1,
        ];

        $vendor_evaluation = [
            'recommended_category' => $basic['recommended_category'] ?? '',
            'evaluation_criteria'   => [
                __( 'Security and compliance', 'rtbcb' ),
                __( 'Integration capabilities', 'rtbcb' ),
                __( 'Total cost of ownership', 'rtbcb' ),
            ],
        ];
        ...
        return [
            ...
            'executive_summary'    => [
                ...
                'business_case_strength'   => __( 'Strong', 'rtbcb' ),
