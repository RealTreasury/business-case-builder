...
class RTBCB_Ajax {
	 ...
	 */
		...
		try {
                       ...
                       if ( $job_id ) {
                               RTBCB_Background_Job::update_status( $job_id, 'processing', [ 'profile' => $enriched_profile ] );
                       }
			...
			$roi_scenarios       = $enhanced_calculator->calculate_enhanced_roi( $user_inputs, $enriched_profile );
                       ...
                       if ( $job_id ) {
                               RTBCB_Background_Job::update_status( $job_id, 'processing', [ 'basic_roi' => $roi_scenarios ] );
                       }
		...
		if ( empty( $job_id ) ) {
			...
			return;
                ...
                foreach ( $status as $key => $value ) {
                        if ( in_array( $key, [ 'status', 'step', 'message', 'percent' ], true ) ) {
                               continue;
                        }
                        $response[ $key ] = $value;
                }
