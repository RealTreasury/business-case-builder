  ...
  <form id="rtbcbForm" class="rtbcb-wizard">
    ...
    <div class="rtbcb-wizard-steps">
      ...
      <div class="rtbcb-wizard-step" data-step="2">
        ...
        <select id="job_title" name="job_title"><option value=""></option><option value="cfo">CFO</option></select>
      </div>
...
(async () => {
  ...
  // job_title intentionally left blank
  await builder.handleNext();

  assert.strictEqual(builder.currentStep, 3, 'Should advance to step 3 without job title');

  const formData = builder.collectFormData();
  assert.strictEqual(formData.get('job_title'), null, 'Empty job title should not be included');
