...
class RTBCB_Admin {
    ...
    /**
     * Render Real Treasury overview test page.
     *
     * @return void
     */
    ...
    public function ajax_test_real_treasury_overview() {
        ...
        $include_portal = isset( $_POST['include_portal'] ) && '1' === sanitize_text_field( wp_unslash( $_POST['include_portal'] ) );
        $raw_categories = isset( $_POST['categories'] ) ? (array) $_POST['categories'] : [];
        $allowed        = array_keys( RTBCB_Category_Recommender::get_all_categories() );
        $categories     = array_filter( array_map( 'sanitize_key', $raw_categories ), function ( $cat ) use ( $allowed ) {
